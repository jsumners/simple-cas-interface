...
it('Supports protocol 2.0', function prot2 (done) {
  ...
  cas
    ._validateServiceResponse(
      fs.readFileSync(path.join(__dirname, '/fixtures/protocol2_success.xml'))
    )
    .then(function resolved (result) {
...
it('Supports protocol 3.0', function prot2 (done) {
  ...
  cas
    ._validateServiceResponse(
      fs.readFileSync(path.join(__dirname, '/fixtures/protocol3_success.xml'))
    )
    .then(function resolved (result) {
      expect(result).to.be.an.object
      expect(result).to.have.property('user')
...
it('Rejects authentication failures', function authfail2 (done) {
  const xml = fs.readFileSync(path.join(__dirname, '/fixtures/protocol2_failure.xml'))
  proto2(xml)
    .then(function resolved () {})
    .catch(function caught (result) {
      expect(result).to.be.an.instanceof(Error)
...
it('Resolves authentication successes', function authyay2 (done) {
  const xml = fs.readFileSync(path.join(__dirname, '/fixtures/protocol2_success.xml'))
  proto2(xml)
...
it('Resolves authentication successes', function authyay2 (done) {
  const xml = fs.readFileSync(path.join(__dirname, '/fixtures/protocol3_success.xml'))
  proto3(xml)
    .then(function resolved (success) {
      expect(success).to.be.an.object
      expect(success).to.have.property('user')
